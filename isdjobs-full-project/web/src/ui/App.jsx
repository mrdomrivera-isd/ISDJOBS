      ...
      {Object.keys(FOCUS).map(k => {
        ...
        return (
          <div key={k}
            ...
            style={{padding:"6px 10px",border:"1px solid #2c3b66",borderRadius:999,cursor:"pointer",background:active?"#2b3a6a":"#121a38"}}>
            {k}
          </div>
        );
      })}
  ...
  return (
    <div className="job" style={{background:"#0f1530",border:"1px solid #263256",borderRadius:10,padding:14}}>
      <div style={{display:"flex",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
        <div>
          ...
          <div style={{color:"#9aa3b2"}}>
            <b>{job.company}</b>{job.location ? ` • ${job.location}`: ""}{job.remote ? " • Remote": ""}{job.posted_at ? ` • ${new Date(job.posted_at).toLocaleDateString()}`: ""}
          </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:10}}>
        <select style={{padding:10,borderRadius:10,border:"1px solid #263256",background:"#0f1530",color:"#f3f5f7"}} value={status} onChange={e=>setStatus(e.target.value)}>
          <option value="">—</option>
        ...
        <input style={{padding:10,borderRadius:10,border:"1px solid #263256",background:"#0f1530",color:"#f3f5f7"}} placeholder="Notes…" value={notes} onChange={e=>setNotes(e.target.value)} />
      </div>
      <div style={{marginTop:8}}>
        <button style={{padding:"10px 12px",borderRadius:10,background:"#3b82f6",border:"1px solid #3b82f6",color:"#fff",cursor:"pointer"}} onClick={()=>onSave(job.url, status, notes)}>Save bookmark</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="container" style={{maxWidth:1100, margin:"0 auto", padding:24}}>
      ...
      <div className="card" style={{marginBottom:16, background:"#11182e", border:"1px solid #1e2747", borderRadius:14, padding:16}}>
        <div className="row" style={{display:"grid", gap:12, gridTemplateColumns:"repeat(12, minmax(0,1fr))", alignItems:"end"}}>
          <div style={{gridColumn:"span 6"}}>
            ...
            <input style={{padding:10,borderRadius:10,border:"1px solid #263256",background:"#0f1530",color:"#f3f5f7", width:"100%"}} placeholder="https://your-api.onrender.com" value={apiBase} onChange={e=>setApiBase(e.target.value)} />
          </div>
          <div style={{gridColumn:"span 2"}}>
            <label style={{color:"#9aa3b2"}}>ZIP</label>
            <input style={{padding:10,borderRadius:10,border:"1px solid #263256",background:"#0f1530",color:"#f3f5f7", width:"100%"}} value={zip} onChange={e=>setZip(e.target.value)} />
          </div>
          <div style={{gridColumn:"span 2"}}>
            <label style={{color:"#9aa3b2"}}>Radius (mi)</label>
            <input style={{padding:10,borderRadius:10,border:"1px solid #263256",background:"#0f1530",color:"#f3f5f7", width:"100%"}} type="number" value={radius} onChange={e=>setRadius(Number(e.target.value||0))} />
          </div>
          <div style={{gridColumn:"span 2"}}>
            <label style={{color:"#9aa3b2"}}>Clearance</label>
            <select style={{padding:10,borderRadius:10,border:"1px solid #263256",background:"#0f1530",color:"#f3f5f7", width:"100%"}} value={requireClearance ? "1":"0"} onChange={e=>setRequireClearance(e.target.value==="1")}>
          ...
          <div style={{gridColumn:"span 6"}}>
            ...
            <input style={{padding:10,borderRadius:10,border:"1px solid #263256",background:"#0f1530",color:"#f3f5f7", width:"100%"}} type="number" value={salaryMin} onChange={e=>setSalaryMin(Number(e.target.value||0))} />
          </div>
          <div style={{gridColumn:"span 4"}}>
            <label style={{color:"#9aa3b2"}}>Salary Max (USD)</label>
            <input style={{padding:10,borderRadius:10,border:"1px solid #263256",background:"#0f1530",color:"#f3f5f7", width:"100%"}} type="number" value={salaryMax} onChange={e=>setSalaryMax(Number(e.target.value||0))} />
          </div>
          <div style={{gridColumn:"span 2"}}>
            <label style={{color:"#9aa3b2"}}>Pay Type</label>
            <select style={{padding:10,borderRadius:10,border:"1px solid #263256",background:"#0f1530",color:"#f3f5f7", width:"100%"}} value={payType} onChange={e=>setPayType(e.target.value)}>
          ...
          <div style={{gridColumn:"span 6"}}>
            ...
            <div style={{marginTop:6}}><FocusChips value={focus} onChange={setFocus} /></div>
          </div>
          <div style={{gridColumn:"span 6"}}>
            <label style={{color:"#9aa3b2"}}>Search (title/company)</label>
            <input style={{padding:10,borderRadius:10,border:"1px solid #263256",background:"#0f1530",color:"#f3f5f7", width:"100%"}} value={query} onChange={e=>setQuery(e.target.value)} placeholder="e.g. Instructional Designer" />
          </div>
          <div style={{gridColumn:"span 6"}}>
            <button style={{padding:"10px 12px",borderRadius:10,background:"#1f2937",border:"1px solid #374151",color:"#f3f5f7",cursor:"pointer"}} onClick={()=> setSalaryMin(100000)}>Use my default (≥$100k)</button>
            <button style={{padding:"10px 12px",borderRadius:10,background:"#1f2937",border:"1px solid #374151",color:"#f3f5f7",cursor:"pointer", marginLeft:8}} onClick={useMyLocation}>Use my location</button>
            {lat && lon ? <span style={{color:"#9aa3b2", marginLeft:8}}>Lat {lat.toFixed(4)} Lon {lon.toFixed(4)}</span> : null}
          </div>
          <div style={{gridColumn:"span 6", textAlign:"right"}}>
            <button style={{padding:"10px 12px",borderRadius:10,background:"#3b82f6",border:"1px solid #3b82f6",color:"#fff",cursor:"pointer"}} onClick={runSearch} disabled={loading}>{loading ? "Searching…" : "Search via API"}</button>
          </div>
        </div>
      ...
      <div className="list" style={{display:"grid", gap:12}}>
        {filtered.length === 0 ? <div style={{color:"#9aa3b2"}}>No results yet. Set API Base and run a search.</div> : null}
        {filtered.map(job => (
